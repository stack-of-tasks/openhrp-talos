...
int main(int argc, char* argv[])
{
  ...
  if (argc!=3)
    {
      std::cout << argv[0] 
		<< " must have two parameter (path to trajectories and pid gains)."
		<< std::endl;
      return -1;
    }
  ...
  try 
    {
      ...
      Controller_impl<TALOSOH2SQP> * talosControllerImpl = new Controller_impl<TALOSOH2SQP>(orb);
      talosControllerImpl->set_path_to_data(trajectory_filename);
      talosControllerImpl->set_path_to_pid_gains(pid_gains_filename);
      talosControllerObj = talosControllerImpl -> _this();
      CosNaming::Name nc;
      nc.length(1);
      nc[0].id = CORBA::string_dup("TALOSPDSequencePlayer");
